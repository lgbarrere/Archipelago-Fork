...
def create_regions(world: MultiWorld, options: CCCharlesOptions, player: int) -> None:
    ...
    rocket_grounds_region = Region("Rocket Grounds", player, world)
    rocket_grounds_region.add_locations(loc_rocket_grounds, CCCharlesLocation)
    world.regions.append(rocket_grounds_region)

    rocket_bunker_region = Region("Rocket Bunker", player, world)
    rocket_bunker_region.add_locations(loc_rocket_bunker, CCCharlesLocation)
    world.regions.append(rocket_bunker_region)
    ...
    temple_region = Region("Temple", player, world)
    ...
    menu_region.connect(military_base_region, "Military Base Fogless")
    menu_region.connect(south_mine_outside_region, "South Mine Outside Fogless")
    south_mine_outside_region.connect(south_mine_inside_region, "South Mine Gate")
    menu_region.connect(middle_station_region, "Middle Station Fogless")
    menu_region.connect(canyon_region, "Canyon Fogless")
    ...
    menu_region.connect(church_region, "Church Fogless")
    ...
    menu_region.connect(mine_elevator_exit_region, "Mine Elevator Exit Fogless")
